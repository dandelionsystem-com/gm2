...
namespace System.gm
{
    public class CryptoExpress
    {
        ...
        public static byte[] Sha512Hash(byte[] ba)
        {
            SHA512Managed sha5 = new SHA512Managed();
            byte[] ba2 = sha5.ComputeHash(ba);
            return ba2;
        ...
        public static string GetNumberHash(string sourceText, int outLength)
        {
            SHA1CryptoServiceProvider sha = new SHA1CryptoServiceProvider();
            byte[] ba = sha.ComputeHash(Encoding.UTF8.GetBytes(sourceText));
            int hashedInt = BitConverter.ToInt32(ba, 0);
            while (hashedInt.ToString().Length > outLength)
            {
                hashedInt = hashedInt / 13;
            }
            while (hashedInt.ToString().Length < outLength)
            {
                hashedInt = hashedInt * 3;
            }

            return hashedInt.ToString();
        }
    }
}
